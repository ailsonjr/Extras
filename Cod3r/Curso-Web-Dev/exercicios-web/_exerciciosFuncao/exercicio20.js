...
let randomMoney = Math.round(Math.random() * (1000 -1)) + 1;

function disposeMoney(totalValue) {
  let [bill100, bill50, bill10, bill5, bill1] = [0, 0, 0, 0, 0];
  console.log(`The requested amount was U$${totalValue.toFixed(2).replace('.',',')}`)
  while (totalValue > 0) {
    if (totalValue / 100 >= 1) {
      bill100 = Math.floor(totalValue / 100);
      totalValue = (totalValue % 100);
    }
    if (totalValue / 50 >= 1) {
      bill50 = Math.floor(totalValue / 50);
      totalValue = (totalValue % 50);
    }
    if (totalValue / 10 >= 1) {
      bill10 = Math.floor(totalValue / 10);
      totalValue = (totalValue % 10);
    }
    if (totalValue / 5 >= 1) {
      bill5 = Math.floor(totalValue / 5);
  ...
  let bills = [bill100, bill50, bill10, bill5, bill1];
  let value = [100, 50, 10, 5, 1]
  let result = '';
  for ( i in bills) {
    if(bills[i] !== 0) {
      result += `${bills[i]} bill(s) of U$${value[i]}. `
    }   
  }
  return result
}

console.log(disposeMoney(randomMoney));
