/* Escreva um algoritmo que leia o código de um aluno e suas três notas. Calcule a média ponderada do aluno, considerando que o peso para a maior nota seja 4 e para as duas restantes, 3. Mostre o código do aluno, suas três notas, a média calculada e uma mensagem "APROVADO" se a média for maior ou igual a 5 e "REPROVADO" se a média for menor que 5. Repita a operação até que o código lido seja negativo. */

let randomNote1 = Math.round(Math.random() * 10);
let randomNote2 = Math.round(Math.random() * 10);
let randomNote3 = Math.round(Math.random() * 10);
let randomStudentCode = Math.round(Math.random() * (100 - 1)) + 1;

function schoolAvarage(studentCode, note1, note2, note3) {
  let approval = '';
  let avarage = ((note1 * 0.4) + (note2 * 0.3) + (note3 * 0.3)).toFixed(1);
  if(avarage >= 5) {
    approval = 'Approded!';
  } else {
    approval = 'Disapproved!'
  }
  return `The student with the code ${studentCode} had the notes ${note1}, ${note2} and ${note3}, averaging ${avarage} and was ${approval}`
}

console.log(schoolAvarage(randomStudentCode, randomNote1, randomNote2, randomNote3));


/* Gabarito

function calcularNotaFinal(codAluno, nota1, nota2, nota3){
    let notas = []
    notas.push(nota1)
    notas.push(nota2)
    notas.push(nota3)
    notas.sort((a,b) => a < b ? 1 : -1)

    let mediaFinal = (notas[0] * 4 + notas[1] * 3 + notas[2] * 3)/10
    console.log(`Código do Aluno: ${codAluno}. Notas: ${nota1}, ${nota2}, ${nota3}. ${mediaFinal < 5 ? 'Reprovado.' : 'Aprovado.'}`)
}

calcularNotaFinal(123, 2.8, 6, 3.5);

*/